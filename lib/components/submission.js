...
const waitingStatusCode = 0; // TODO: build a constant table for this
...
const promises = {
    ...
    getAmountOfWaitingSubmission: (uid, pid) => {
        return new Promise((resolve, reject) => {
            const cmd = 'SELECT count(*) AS amount FROM submissions WHERE uid = ? AND pid = ? AND res = ?';

            connection.query(cmd, [uid, pid, waitingStatusCode], (err, result) => {
                if (err)
                    reject(err);
...
function lgnToAnonymous(lgn) {
    const xMarkMaxLen = 3;

    let replaceLen = 0;
    while (replaceLen < xMarkMaxLen) {
        if (lgn.length == replaceLen)
            break;

        let c = lgn[lgn.length - 1 - replaceLen];
        if (c < '0' || c > '9')
            break;
    ...
    lgn = lgn.slice(0, lgn.length - replaceLen);
...
module.exports = {
    ...
    listinfo: function(filter, isAdmin, isStrong, callback) {
        // to provide pagination info in the submission page
    ...
    toAnonymousList: (list, uid, isAdmin) => {
        return list.map(row => {
            if (row.uid == uid)
                return row;
            ...
            if (!isAdmin)
                row.uid = -1;
            row.lgn = lgnToAnonymous(row.lgn);
