...
module.exports = {
    isAuthenticated: function(req, res, next) {
		if (req.session && req.session.uid) {
            var cmd = 'SELECT uid FROM users WHERE uid = ?';
            connection.query(cmd, [req.session.uid], function(err, result) {
    ...
    isStrong: function(uid, callback) {
        var cmd = 'SELECT uid, class FROM users WHERE uid = ?';
        connection.query(cmd, [uid], function(err, result) {
            if (err || result.length == 0)
                return callback(false);

            return callback(result[0]["class"] == 5);
        });
    },
    ...
    cansubmit: function(cid, pid, uid, callback) {
        ...
        if (cid == 0) {
            ...
            connection.query(cmd, [pid], function(err, result) {
                if (err || result.length == 0) {
                    loggerFactory.getLogger(module.id).debug(new Error('can submit 1'), { err, cid, pid, uid});
                    callback(false);
                    return 0;
                }
            ...
            connection.query(cmd, [cid], function(err, result) {
                if (err || result.length == 0) {
                    loggerFactory.getLogger(module.id).debug(new Error('can submit 2'), { err, cid, pid, uid});
                    callback(false);
                    return 0;
                }
                var st, ed;
                ...
                connection.query(cmd, [cid, uid], function(err, result) {
                    if (err) {
                        loggerFactory.getLogger(module.id).debug(new Error('can submit 3'), { err, cid, pid, uid});
                        callback(false);
                        return 0;
                    }
            ...
            connection.query(cmd, [cid, pid], function(err, result) {
                ...
                connection.query(cmd, [cid], function(err, result) {
                    ...
                    connection.query(cmd, [cid, uid], function(err, result) {
                        ...
                        loggerFactory.getLogger(module.id).debug('can read !!!!!!!');
                        return callback(true);
                    });
                });
            });
